        ...
        this.videoEncoder = new VideoEncoder({
          output: (chunk, metadata) => {
            try {
              ...
              if (chunk && chunk.type) {
                // Extract binary data from EncodedVideoChunk into Uint8Array
                const uint8Array = new Uint8Array(chunk.byteLength);
                chunk.copyTo(uint8Array.buffer);
                this.recordedChunks.push(uint8Array);
                if (this.recordingFramesEncoded % 10 === 0) {
                  console.log(`[OUTPUT] Frame ${this.recordingFramesEncoded}, chunk byteLength: ${chunk.byteLength}, array size: ${uint8Array.byteLength}`);
                }
              }
            } catch (e) {
